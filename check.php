									...
									<div class="card-body pb-0" >
										<div class="table-responsive height720 dz-scroll" id="patient-activity">
											<?php
												...
												foreach($check_sql as $row) :
												endforeach;

												$name = $row['fname'];
												$query = "SELECT * FROM otherappointment WHERE doctor = '$name'";
												$qry = mysqli_query($db,$query);
												foreach($qry as $rows) :
												endforeach;
												...
												$diffDate = abs($curr - $datee);
												function dateDif($datee,$curr){
													$differ = $datee - $curr;
													return abs(round($differ / 86400));
												}
														
												$dateDiff = dateDif($datee, $curr);
															...
															<div class="d-flex">
																...
																<div>
																	<?php
																		$fname = $row['fname'];
																	?>
																	<h6 class="fs-16 mb-1"><a href="#" class="text-black"><?php echo $row['fname'] . ' ' . $row['lname']; ?></a></h6>
																	<span class="fs-14">
																		<?php
																			require 'connection.php';
																			$fname = $row['fname'];
															...
															<div>
																...
																<p class="mb-0 fs-12">
																	<?php
																		...
																		function dateDiff($datee,$curr){
																			$differ = $datee - $curr;
																			return abs(round($differ / 86400));
																		}
																		
																		$dateDiff = dateDiff($datee, $curr);
																		
																		echo $rows['appdate'] . '-' . $rows['apptime'];
